...
const WorkshopModal = ({ workshop, isOpen, onClose }: { 
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg max-w-4xl w-full m-4 flex flex-col max-h-[90vh]">
        {loading ? (
          ...
          <div className="p-8 text-center">
            <h2 className="text-xl font-semibold text-gray-700">No Workshop Data Found</h2>
            <p className="text-gray-500 mt-2">The workshop details are currently unavailable In Squidex.</p>
            <button 
              onClick={onClose}
          ...
          <>
            {/* Header Section with Workshop Image */}
            <div className="p-8 border-b">
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-2xl font-bold text-gray-900">{workshopDetails?.displayName}</h2>
            ...
            <div className="p-8 overflow-y-auto flex-1">
              <div className="space-y-6">
                {/* Workshop Gallery */}
                {workshopDetails?.galleryImages && workshopDetails.galleryImages.length > 0 && (
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <h3 className="text-lg font-semibold mb-3">Workshop Gallery</h3>
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {workshopDetails.galleryImages.map((imageId: string, index: number) => (
                        <img
                          key={index}
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="flex items-center">
                      ...
                      <span className="ml-2">{workshopDetails?.rating} ({workshopDetails?.reviewsCount} reviews)</span>
                    </div>
                    <div className="flex items-center">
                      <Calendar className="h-5 w-5 text-blue-500 mr-2" />
                      <span className="font-medium">Founded:</span>
                      <span className="ml-2">{workshopDetails?.businessStartedYear}</span>
                    </div>
                    <div className="flex items-center">
                    ...
                    {workshopDetails?.franchiseDetails && workshopDetails.franchiseDetails.length > 0 && (
                      <div className="col-span-2">
                        <span className="font-medium">Franchises:</span>
                        <div className="flex gap-4 mt-2">
                          {workshopDetails.franchiseDetails.map((franchise:any, index:number) => (
                    ...
                    {workshopDetails?.customer?.[0] && (
                      <div className="col-span-2">
                        ...
                        >
                          ...
                          <span className="text-gray-500">{isOwnerDetailsOpen ? '−' : '+'}</span>
                        </button>
                        {isOwnerDetailsOpen && (
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="bg-white p-3 rounded-lg shadow-sm">
                        <div className="text-blue-600 font-semibold">Vehicles Serviced</div>
                        <div className="text-2xl font-bold">{workshopDetails.workshopServiceHistory[0].vehiclesServiced}</div>
                      </div>
                      <div className="bg-white p-3 rounded-lg shadow-sm">
                        <div className="text-green-600 font-semibold">Cashless Tie-ups</div>
                        <div className="text-2xl font-bold">{workshopDetails.workshopServiceHistory[0].cashlessTieUps}</div>
                      </div>
                      <div className="bg-white p-3 rounded-lg shadow-sm">
                        <div className="text-purple-600 font-semibold">Specialized Services</div>
                        <div className="text-2xl font-bold">{workshopDetails.workshopServiceHistory[0].specializedServices}</div>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {/* Brands Section */}
                {workshopDetails?.brands && workshopDetails.brands.length > 0 && (
                  <div className="bg-gray-50 p-4 rounded-lg">
                ...
                {/* Working Hours */}
                {workshopDetails?.workshopAvailableTimes && workshopDetails.workshopAvailableTimes.length > 0 && (
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <h3 className="text-lg font-semibold mb-3">Working Hours</h3>
                    <div className="flex items-center text-gray-700">
                      <Clock className="h-5 w-5 mr-2 text-gray-500" />
                      {workshopDetails.workshopAvailableTimes[0].startTime} - {workshopDetails.workshopAvailableTimes[0].endTime}
                    </div>
                  </div>
                )}
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg mt-6">
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-500">({servicesData.length})</span>
                      </div>
                        ...
                        {servicesData
                          ...
                          <div key={service.id} className="bg-white p-4 rounded-lg shadow-sm">
                            ...
                            <div className="space-y-2">
                              {service.data.features?.slice(0, 3).map((feature: any, index: number) => (
                                <div key={index} className="flex items-start text-sm">
                                  <Wrench className="h-4 w-4 text-gray-400 mr-2 mt-1" />
                                  <div>
                                    <span className="font-medium">{feature.item}</span>
...
export const WorkshopTable: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        setLoading(true);
        const response = await fetch(
          "http://127.0.0.1:5000/getDashboardData"
        );
        if (!response.ok) {
      ...
      case "workshopId":
        return (
          <button
            onClick={() =>
              handleWebsiteClick(
                workshop["squidexWorkshopWebsiteLink"] + ""
              )
            }
            className="text-blue-600 hover:text-blue-800 underline flex items-center gap-1"
          >
  ...
  const [selectedWorkshop, setSelectedWorkshop] = useState<Workshop>(workshopData[0]);
